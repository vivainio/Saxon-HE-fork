 ...
 */
public class RuleSetWithTypeCheck implements BuiltInRuleSet {


    private final BuiltInRuleSet baseRuleSet;
    private final SequenceType requiredType;

    public RuleSetWithTypeCheck(BuiltInRuleSet baseRuleSet, SequenceType requiredType) {
     ...
     * @param locationId   location of the instruction (apply-templates, apply-imports etc)
     * @throws XPathException if the output of the built-in template rule does not match the
     * required type
     */
    ...
    public void process(Item item, ParameterSet parameters,
        ...
        RoleDiagnostic role = new RoleDiagnostic(RoleDiagnostic.TEMPLATE_RESULT, "<built-in>", 0);
        filter.setRequiredType(requiredType.getPrimaryType(), requiredType.getCardinality(), role, locationId);
        baseRuleSet.process(item, parameters, tunnelParams, filter, context, locationId);
