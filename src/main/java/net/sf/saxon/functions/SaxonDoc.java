 ...
 * instruction with streamable="no". A lot of this is superseded by the fn:doc#2 function
 * in QT4.
 */

public class SaxonDoc extends SystemFunction implements Callable {
    ...
    public static ParseOptions setParseOptions(
        ...
        if (value != null) {
            ...
            result = result
                    .withSchemaValidationMode(v)
                    .withSchema(rsc.getImportedSchema());
        }
        value = checkedOptions.get("type");
        if (value != null) {
            QNameValue qval = (QNameValue) value.head();
            StructuredQName name = qval.getStructuredQName();
            result = result
                    .withTopLevelType(rsc.getImportedSchema().getSchemaType(name))
        ...
        if (value != null) {
            ...
            switch (s) {
                case "all":
                    result = result.withSpaceStrippingRule(AllElementsSpaceStrippingRule.INSTANCE);
                    break;
                case "none":
                    result = result.withSpaceStrippingRule(NoElementsSpaceStrippingRule.INSTANCE);
                    break;
                case "ignorable":
                    result = result.withSpaceStrippingRule(IgnorableSpaceStrippingRule.INSTANCE);
                    break;
    ...
    public Sequence call(XPathContext context, Sequence[] arguments) throws XPathException {
        ...
        Map<String, GroundedValue> checkedOptions =
                getDetails().optionDetails.processSuppliedOptions((MapItem) param, context, 40);
    ...
    private TreeInfo fetch(String href, ParseOptions options, XPathContext context) throws XPathException {
        ...
        Source source = config.getSourceResolver().resolveSource(new StreamSource(abs.toASCIIString()), options, config);
