...
/**
 * A collating sequence that compares strings according to the Unicode rules for case-blind matching.
 */

public class UnicodeCaseBlindCollator implements StringCollator, SubstringMatcher {

    private static final UnicodeCaseBlindCollator theInstance = new UnicodeCaseBlindCollator();

    public static UnicodeCaseBlindCollator getInstance() {
        return theInstance;
    }
    ...
    public String getCollationURI() {
        return NamespaceConstant.UNICODE_CASE_BLIND_COLLATION_URI;
    }
    ...
    private int compareCS(UnicodeString a, UnicodeString b) {
        return CodepointCollator.getInstance().compareStrings(normalize(a), normalize(b));
    }
